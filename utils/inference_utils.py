...
def return_sen_to_real_form(tokenizer, input_sen, tags):
    ...
    word_ids_tags = [(x, tags[i]) for i,x in enumerate(encoded.word_ids(batch_index=0)) if x is not None]
    
    word_ids = [x[0] for x in word_ids_tags]
    _tags = [x[1] for x in word_ids_tags]
    ...
    for i in range(num_words):
        ...
        if _tags[index_in_tag_seq] == 'e':
            out += 'ِ '
        elif _tags[index_in_tag_seq] == 'ye':
