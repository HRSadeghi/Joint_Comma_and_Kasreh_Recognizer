...
class JointBERTBiLSTMTagger(nn.Module):
    ...
    def __init__(self,
        ...
        self.lstm2 = nn.LSTM(bert_out_dim, lstm_dim, bidirectional=True, num_layers=1)
        self.bert_model = bert_model
        self.linear1 = nn.Linear(2*lstm_dim, num_classes)
        self.linear2 = nn.Linear(2*lstm_dim, 2)
    ...
    def forward(self, input):
        ...
        kasreh_lstm_output, _ = self.lstm1(hidden)
        kasreh_lstm_output = self.dropout_layer1(kasreh_lstm_output)
        
        kasreh_logits = self.linear1(kasreh_lstm_output)


        comma_lstm_output, _ = self.lstm2(hidden)
        kasreh_lstm_output = self.dropout_layer2(comma_lstm_output)
        
        comma_logits = self.linear2(comma_lstm_output)

        return kasreh_logits, comma_logits
