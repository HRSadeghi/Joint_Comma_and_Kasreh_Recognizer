...
def remove_comma_from_sens_tags(sens, tags):
    _sens = []
    _tags = []
...
def comma_tag_finder(sens):
    def sample_comma_tag_finder(s):
        out = ['C' if x == '،' else 'N' for x in s]
        out2 = []
        j = 0
        while j < len(out):
            if out[::-1][j] == 'N':
    ...
    for s in sens:
        comma_labels += [sample_comma_tag_finder(s)]

    return comma_labels


def kasre_and_comma_labels_size_same(sens, tags, comma_tags):
    _sens, _tags, _comma_tags = [], [], []
    for i in range(len(tags)):
        if len(comma_tags[i]) == len(tags[i]):
            _sens.append(sens[i])
            _tags.append(tags[i])
            _comma_tags.append(comma_tags[i])
    return _sens, _tags, _comma_tags
...
def prepare_dataset_for_train(path, 
    ...
    _comma_tags = comma_tag_finder(_sens)
    _sens, _tags = remove_comma_from_sens_tags(_sens, _tags)


    _sens, _kasreh_tags, _comma_tags = kasre_and_comma_labels_size_same(_sens, _tags, _comma_tags)

    return _sens, _kasreh_tags, _comma_tags
